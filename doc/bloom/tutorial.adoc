...
using some internal streamlining techniques able to reduce execution
times by around 30% to 50%. Similarly, `may_contain` can be executed
...
xref:filter_bulk_may_contain_size[`bulk_may_contain_size`] elements. As with
bulk insertion, execution times can decrease by 30% to 50% depending on
the filter configuration and its capacity (the more memory the filter array
takes, the larger the speed improvements).
