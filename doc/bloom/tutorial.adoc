...
The contents of the bit array can be accessed directly with the `data`
...
out.pass:[write((]const char*) &c1, sizeof(c1)); // save capacity (bits)
out.pass:[write((]const char*) f1.data(), c1 / CHAR_BIT); // save array
out.close();
...
std::size_t c2;
in.pass:[read((]char*) &c2, sizeof(c2));
f2.reset(c2); // restore capacity
in.pass:[read((]char*) f2.data(), c2 / CHAR_BIT); // load array
in.close();
-----

Note that capacities are measured in bits whereas `data()` is an array
of `unsigned char`+++s+++, so its size is `capacity() / CHAR_BIT`.
