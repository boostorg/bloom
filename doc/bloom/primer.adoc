...
{small}stem:[1/2^{k_{\text{opt}}} \approx 0.6185^{m/n}]{small-end}. See the section
...
operation performance. A variation on the classical approach called a
...
image::block_multiinsertion.png[align=center, title="Block filter with multiinsertion."]
...
Multiblock filters can also be combined with multiinsertion. In general,
...
multiinsertion, the number of available configurations grows quickly and
