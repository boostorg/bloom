...
public:
  ...
  // data access
  unsigned char*       xref:#filter_data[data]() noexcept;
  const unsigned char* xref:#filter_data[data]() const noexcept;
...
[horizontal]
Postconditions:;; [`data()`, `data() + capacity() / CHAR_BIT`) is a valid range.
